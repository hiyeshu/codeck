...
description: |
  ...
  DeckSpec and compiles HTML. Outputs $DECK_DIR/deck.json + deck.html. Use whenever
...
最终 HTML = `render-default $DECK_DIR/deck.json` 先生成 `$DECK_DIR/default.html`，再由 Claude 读取 `$DECK_DIR/default.html` + `$DECK_DIR/design.json` + Lisp prompt，直接输出完整的自包含 HTML 文件。
...
[ -f ~/.claude/skills/codeck/skill/compiler/index.ts ] && echo "COMPILER: FOUND" || echo "COMPILER: MISSING"
```
...
如果 `COMPILER: MISSING`，报错停止：`❌ 缺少 compiler/index.ts。请确认 codeck 技能已正确安装。`
...
收束后优先走新的最终渲染管线；如果 compiler 不可用或 `render-default` 失败，回退到旧的 `shell.html` 模板路径：
...
5. `write-final $DECK_DIR/deck.json $DECK_DIR/{title}-r{revision}.candidate.html $DECK_DIR/default.html` → 自动按 `meta.title + revision` 生成真正的 `{title}-r{revision}.html` 并落盘
6. `write-final` 内部先做 `validate-html`，校验 slide / block / shell contract
7. 如果校验失败：把错误摘要反馈给 Claude，基于同一 prompt 修复一次，再次执行 `write-final`
8. 如果 `render-default` 本身失败（exit code != 0）：回退到读取项目根目录 `shell.html`（若不存在则读 `~/.claude/skills/codeck/shell.html`）的旧渲染路径，沿用旧的 final HTML 生成方式，避免整条 design 流程阻塞

渲染流程：

1. **validate / 收束** — deck.draft.json 先校验 schemaVersion 2 与 traverse 合同
2. **默认 renderer** — compiler registry 把 spec 渲成默认版 HTML，Deck 组件提供翻页/全屏/备注/帮助等交互
...
7. **fallback** — 如果 compiler 渲染失败，回退到 `shell.html` 模板路径继续旧渲染链路
8. **写出文件** — `write-final` 自动从 `$DECK_DIR/deck.json` 的 `meta.title` + `revision` 生成最终文件名，如 `重新定义可能-r1.html`
...
```bash
DECK_DIR="$DECK_DIR" npx tsx ~/.claude/skills/codeck/skill/pipeline.ts done design
```
