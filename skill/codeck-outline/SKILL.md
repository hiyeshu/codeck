...
先检查 `$DECK_DIR/scan.json`（由 `/codeck` 入口生成）：
- 如果存在且 `scannedAt` 在 30 分钟内 → 直接读缓存，跳过下面的 find 命令
- 如果不存在或过期 → 执行下面的扫描
...
**图片资产处理：** 如果扫描到图片文件，复制到 `$DECK_DIR/assets/` 并记录资产清单：
```bash
mkdir -p "$DECK_DIR/assets"
# 对每个扫描到的图片
cp "{image_path}" "$DECK_DIR/assets/"
```

资产清单格式（写入 outline.md 的「资产清单」部分）：
```json
{
  "assets": [
    { "file": "assets/architecture.png", "original": "./docs/architecture.png" },
    { "file": "assets/screenshot.jpg", "original": "./images/screenshot.jpg" }
  ]
}
```
...
推荐 block：statement-hero → problem-card-grid → bullet-list → code → chart → callout
```

**Demo 驱动：** 适合产品演示、功能介绍
```
封面 → 概念（一句话） → 演示截图/视频 → 原理解析 → 对比效果 → 扩展场景
推荐 block：hero → image/video → flow → comparison → chart
```

**数据报告：** 适合周报、月报、数据分析
```
封面 → 摘要结论（前置） → 关键指标 → 趋势分析 → 归因分析 → 行动建议
推荐 block：statement-hero → metric-grid/dark-stat → chart → callout → bullet-list
```

**教学/培训：** 适合内部培训、知识分享
```
封面 → 为什么学这个 → 核心概念 → 逐步讲解 → 代码示例 → 练习/作业
推荐 block：hero → bullet-list → flow → code → callout
```
...
## Step 5: 写出 $DECK_DIR/outline.md + $DECK_DIR/outline.json

用 Write 工具创建 `$DECK_DIR/outline.md`，这里只保留自然语言内容：
...
{
  "assets": [
    { "file": "assets/{filename}", "original": "{原始路径}" }
  ]
}
```
如果没有图片，写 `无图片资产。`
...
然后用 Write 工具创建 `$DECK_DIR/outline.json`，写结构化 handoff：
...
如果没有图片资产，`assets` 可以省略。

## Step 5.5: 写出 $DECK_DIR/intent.md + $DECK_DIR/intent.json

**如果是重跑（intent.md 已存在）：** 先清除所有已有的角色笔记段落——用 Edit 工具删除从 `## 编辑笔记` / `## 设计师笔记` / `## 审稿人笔记` / `## 演讲稿笔记` 开头到下一个 `##` 之间的内容。保留 intent.md 的其他内容（核心动机、受众画像等）。
...
```bash
DECK_DIR="$DECK_DIR" npx tsx ~/.claude/skills/codeck/skill/intent-schema.ts create "$DECK_DIR/intent-input.json"
```

字段值要求：
- `constraints` 填硬约束；没有就传 `[]`
- `must_include` / `must_avoid` / `open_questions` / `working_assumptions` 没有就传 `[]`
- `tone` 没探索到就写 `未指定`
- `narrative_arc` 只用四个枚举：`problem-driven` / `demo-driven` / `data-report` / `teaching`
...
```bash
DECK_DIR="$DECK_DIR" npx tsx ~/.claude/skills/codeck/skill/pipeline.ts done outline
```
