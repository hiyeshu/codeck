...
codeck/scripts/resolve-dirs.sh: Shared lane-directory resolver sourced by every SKILL.md Setup bootstrap; derives CODECK_ROOT and sibling lane dirs (design/export) with env-override-first, flat-install fallback.
...
- 2026-08-08: Plugin-form restructure (cowart-style self-marketplacing repo). The repo is now simultaneously a Claude Code plugin (`.claude-plugin/plugin.json` + `marketplace.json`, source "./"), a Codex plugin (`.codex-plugin/plugin.json` + `.agents/plugins/marketplace.json`, local source "."), and the existing flat skills bundle. All 8 Setup probe blocks replaced with one unified bootstrap (env override → `${CLAUDE_PLUGIN_ROOT}` → plugin cache globs → flat probes) that sources the new `codeck/scripts/resolve-dirs.sh` sibling resolver. Fixed the hard-coded `$HOME/.claude/skills/` checklist path in codeck-outline (now `$CODECK_ROOT`-relative). CI added: `scripts/check-plugin-metadata.mjs` keeps the four manifests, skill frontmatter versions, and the no-hard-coded-paths rule in lockstep. Bundle version 2.5.0; per-skill patch bumps (codeck 2.4.1, outline 2.2.1, design 2.0.1, review 2.1.1, speech 2.1.1, export 2.1.1). READMEs (all 5 languages) document three install channels plus the clone-and-edit-live workflow.
