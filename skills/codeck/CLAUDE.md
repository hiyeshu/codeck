# codeck/
> L2 | 父级: /Users/gaoding/Desktop/codeck/skills/CLAUDE.md

成员清单
SKILL.md: 用户可触发入口，保留 routing、setup、runtime 指针，不承载长参考。
references/: 分支参考层，按 workflow/design/delivery 拆分长规则。
scripts/: 确定性工具层，负责 assemble、build、validate、export、thumbnail 与浏览器 runtime。
assets/: 固定运行时资源层，负责 base/editor CSS、toolbar/presenter/editor HTML 片段和 icon sprite。

架构决策
`SKILL.md` 是门面；`references/` 是渐进披露；`scripts/` 消除重复生成代码；`assets/` 保存官方规范允许的静态资源，并在构建时内联进单 HTML。
