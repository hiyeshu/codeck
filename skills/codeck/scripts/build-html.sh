...
# [POS]: skills/codeck/scripts final-build guard; wraps assemble.sh so speaker mode cannot be skipped.
...
grep -q 'ck-tpl-toolbar' "$out" || { echo "ERROR: missing toolbar template marker: ck-tpl-toolbar" >&2; exit 1; }
grep -q 'ck-tpl-editor-toolbar' "$out" || { echo "ERROR: missing editor template marker: ck-tpl-editor-toolbar" >&2; exit 1; }
grep -q 'icon-grid' "$out" || { echo "ERROR: missing icon sprite marker: icon-grid" >&2; exit 1; }
