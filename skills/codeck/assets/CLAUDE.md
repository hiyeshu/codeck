...
editor.css: 固定 editor chrome 样式，覆盖 editor toolbar、agent marker dialog、selected node overlay、image slot、mark mode 和 annotation。
toolbar.html: 主放映工具栏 HTML fragment，构建时内联为 `TOOLBAR_HTML`。
presenter.html: presenter mode HTML fragment，构建时内联为 `PRESENTER_HTML`。
editor-toolbar.html: editor mode HTML fragment，构建时内联为 `EDITOR_TOOLBAR_HTML`，包含 Agent marker 入口。
icons.svg: 固定 SVG symbol sprite，供所有 runtime controls 与 agent marker 入口使用。
...
2026-06-27: 新增右侧 agent marker dialog 样式与 toolbar 入口。
