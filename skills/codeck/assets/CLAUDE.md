...
editor.css: 固定 editor chrome 样式，覆盖 editor toolbar、image slot、mark mode 和 annotation。
toolbar.html: 主放映工具栏 HTML fragment，构建时内联为 `TOOLBAR_HTML`。
presenter.html: presenter mode HTML fragment，构建时内联为 `PRESENTER_HTML`。
editor-toolbar.html: editor mode HTML fragment，构建时内联为 `EDITOR_TOOLBAR_HTML`。
icons.svg: 固定 SVG symbol sprite，供所有 runtime controls 使用。
