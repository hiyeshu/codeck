...
读取 `$DECK_DIR/outline.md` — 页面结构、内容要点、用户意图、给设计师的话。
...
## design-dna：同构映射 → 结构化设计档案

这是你的核心工具。两步走：先找同构映射（概念层），再输出 design-dna.json（规格层）。
...
如果内容结构简单（平铺并列），不需要强行找同构。

### Step 2: 生成 design-dna.json
...
| 维度 | 内容 | 来源 |
|------|------|------|
| `design_system` | 配色、字体、间距、布局、形状、阴影、幻灯片类型样式 | 同构映射的具体视觉策略 |
| `design_style` | 情绪、视觉语言、构图、负空间 | 角色审美 + 同构映射 |
| `visual_effects` | 背景效果、文字效果、fragment 入场、玻璃效果 | 内容需要的视觉增强 |

用 Write 工具写入 `$DECK_DIR/design-dna.json`。

**关键约束：**
- 演示文稿字号：标题 48-96px，正文 24-32px，注释 16-20px（不是 web 尺度）
- 系统字体栈，不引用 CDN
- visual_effects 用 CSS 实现，不引入外部库（引擎管交互，AI 管视觉）
- 不适用的字段写 `"none"` 或 `false`

## 风格确认

基于 design-dna.json，向用户描述视觉方向：
