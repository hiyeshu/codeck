...
- **Elements should match the canvas scale.** AI's #1 spatial failure: content that should be prominent ends up tiny — 60px icons on a 1280px canvas, 200px card grids surrounded by emptiness. Use relative units (`vw`, `vh`, `%`, `clamp()`) for visual elements so they breathe with the viewport. `width: 60px` is a speck; `width: min(15vw, 200px)` adapts. Fixed `px` is for borders, gaps, and type — not for cards, icons, diagrams, or grids. Big whitespace is fine when it's a deliberate breathing page (one sentence, one image). It's a problem when multiple elements are present but undersized.
