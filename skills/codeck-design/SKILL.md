...
[ -f "$CODECK_SKILLS/compiler/index.ts" ] && echo "COMPILER: FOUND" || echo "COMPILER: MISSING"
```
...
8. 如果 `render-default` 本身失败（exit code != 0）：回退到读取项目根目录 `shell.html`（若不存在则读 `$CODECK_REPO/shell.html`）的旧渲染路径，沿用旧的 final HTML 生成方式，避免整条 design 流程阻塞
