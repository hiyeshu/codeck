...
- AskUser appears only at allowed decision moments: Project Init, Deck Intent, Design Direction, Export Format, Speech Style.
- Each `/codeck` run has a hard cap of 2 AskUser rounds. Infer first, default second, ask only when the answer changes the deck.
- AskUser shape: Re-ground, Current read, Recommendation, 2-3 mutually exclusive options.
- Record AskUser answers and assumed defaults in `MEMORY.md`; record deck-shaping choices in `deck.md`.
