...
这些问题不强制回答。用户说"没什么特别的"就跳过。答案记录到 outline.md 的"用户意图"段。

**快速模式 / 全跳规则：** 跳过 Q1.5，用户意图段写"未探索"。
...
- 核心动机: {Q1.5 "为什么在乎"的原话。未问到写"未探索"}
- 偏好与禁忌: {喜欢/讨厌的表达方式。未提到写"未指定"}
- 情绪基调: {想让观众产生的感觉。未提到写"未指定"}
...
>
> 产出：`$DECK_DIR/outline.md`
> 下一步：`/codeck-design` — 设计师会读你的大纲，把结构变成视觉。

## 自审（出口前必做）

读取 `$HOME/.claude/skills/codeck-outline/references/checklist.md`，逐项检查 `$DECK_DIR/outline.md`。
