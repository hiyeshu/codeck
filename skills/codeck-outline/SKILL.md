...
1. **图片**（png/jpg/svg/webp/gif）→ 复制到 `$DECK_DIR/assets/`，assemble.sh 会自动内联为 base64
2. **代码片段** → 在 outline.md 对应页面标注文件路径和行号范围，设计师用 `<pre><code>` 嵌入
3. **数据**（csv/json）→ 在 outline.md 标注关键指标，设计师转为表格或 CSS 图表

```bash
mkdir -p "$DECK_DIR/assets"
# 复制内容相关的图片（架构图、截图、数据图表）
# 跳过装饰图（favicon、logo 等无内容价值的）
...
| 文件 | 类型 | 用途 | 分配到页面 |
|------|------|------|-----------|
| assets/architecture.png | 图片 | 架构图 | 第 3 页 |
| src/core.ts:15-30 | 代码 | 核心函数 | 第 5 页 |
| data/metrics.csv | 数据 | 月度指标 | 第 4 页 |
